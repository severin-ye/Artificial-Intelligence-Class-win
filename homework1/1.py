...
# 现在，housing_with_dummies 包含了原始数值列和转换后的分类列
# 重新计算相关性矩阵
corr_matrix = housing_with_dummies.corr()

# 打印 'median_house_value' 与其他列的相关性
...
# 修改 FutureWarning 警告的代码
sample_incomplete_rows["total_bedrooms"] = sample_incomplete_rows["total_bedrooms"].fillna(median)

# 修改 TypeError 错误的代码
# 确保 sklearn 版本是最新的，以支持 sparse 参数
from sklearn.preprocessing import OneHotEncoder

# 如果更新 sklearn 后仍然遇到 TypeError，尝试移除 sparse 参数
cat_encoder = OneHotEncoder()
housing_cat_1hot = cat_encoder.fit_transform(housing_cat)
housing_cat_1hot
...
# 创建SVR模型实例，设置正则化参数C=30000，使用线性核
svm_reg = SVR(C=30000, kernel="linear")
# 使用预处理过的房价数据(housing_prepared)和房价标签(housing_labels)来训练模型
...
# 使用交叉验证计算SVR模型的性能，这里使用3折交叉验证
svm_scores = cross_val_score(svm_reg, housing_prepared, housing_labels,
                             scoring="neg_mean_squared_error", cv=3) # neg_mean_squared_error是负的均方误差
# 交叉验证得分是负的均方误差，取负值后计算每个分数的平方根得到RMSE
svm_rmse_scores = np.sqrt(-svm_scores)

# 打印交叉验证得到的SVR模型的RMSE分数
...
# 比较Gradient Boosting模型和之前SVM模型的RMSE平均值，判断哪个模型表现更好
if gbr_rmse_scores.mean() < svm_rmse_scores.mean():
    # 如果Gradient Boosting模型的RMSE平均值更低，打印出改进的量
    print('成功！您的RMSE改进量：{0:0.3f}'.format(svm_rmse_scores.mean() - gbr_rmse_scores.mean()))
else:
    # 如果SVM模型的RMSE平均值更低，打印出减少的量
    print('尝试不同的方法！您的RMSE减少量：{0:0.3f}'.format(svm_rmse_scores.mean() - gbr_rmse_scores.mean()))
#################################
# ★★★ capture the below result in the report ★★★
